    ...
    try:
        ...
        if get_mode(sender_id) == '1':
            ...
            if get_keyword_ready(sender_id) == '1':
                ...
                set_mode(sender_id, '1')  # 쓰기 모드로 전환합니다.
                ...
                response = tomorrow_weather(parameter.get('date-time.original'), parameter.get('korean_geo.original'))

        return make_response(jsonify({'fulfillmentText': response}))
    except AttributeError:
        return make_response(jsonify({'fulfillmentText': '서버 오류입니다. 2521um@gamil.com 으로 제보해주세요.'}))
